...
activitiModeler.factory('httpInterceptor', [function (authService){
    return {
        request: function (config){
            if(config.method==="GET" && !config.cache){
                ...
                config.params['_btv'] = new Date().getTime();
            }
            return config;
        }
    };
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId,token) {
                ...
                $http({method: 'GET', url: modelUrl,headers:{"access_token":token}}).
